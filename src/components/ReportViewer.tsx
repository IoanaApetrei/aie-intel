...
export default function ReportViewer({ source, date }: Props) {
  ...
  return (
    <div className="flex h-[calc(100vh-56px)]">
      ...
      {/* ── Fixed sidebar ───────────────────────────── */}
      <aside className="hidden lg:flex flex-col w-60 shrink-0 border-r border-gray-800 bg-[#0D1117] overflow-y-auto">
        <div className="px-4 pt-5 pb-2">
          <p className="text-[10px] font-semibold text-gray-500 uppercase tracking-widest">Sections</p>
        </div>
        <nav className="flex-1 px-2 pb-6 space-y-0.5">
          <a
            href="#tldr"
            className="flex items-center gap-2.5 px-3 py-2 rounded-md text-sm text-gray-400 hover:bg-gray-800 hover:text-gray-100 transition-colors"
          >
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        {/* TL;DR hero */}
        <section id="tldr" className="mx-8 mb-2 rounded-xl border border-blue-900 bg-[#0F1D2F] px-8 py-6">
        ...
        {parsed.sections.map(section => (
          <section key={section.id} id={section.id} className="mx-8 mb-1 border-t border-gray-800 py-8">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-8 h-8 rounded-lg bg-gray-800 flex items-center justify-center text-lg shrink-0">
                {section.emoji}
              </div>
